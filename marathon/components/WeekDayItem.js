...
export default function WeekDayItem({ day, isToday, distance }) {
  ...
  return (
    <div className="my-2 py-4 flex w-full justify-between border-b-2 border-gray-100">
      {/* day = 1-7 */}
      <div>
        <p className="text-med font-bold">{weekday[day - 1]}</p>
      </div>
      <div className=" flex flex-row ">
        <p className="pr-4">{distance === "rest" ? "" : distance + " Miles"}</p>
        {renderType(distance)}
        <button>
          <MoreVertIcon />
        </button>
